...
export default function GirisKayit() {
  ...
  const I = {
    width: '100%', padding: '11px 14px', background: '#f5f4f0',
    border: '1px solid #e8e6e0', borderRadius: '10px', fontSize: '14px',
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '20px', padding: '36px', width: '100%', maxWidth: '400px' }}>
        ...
        {mod !== 'sifre' && (
          <div style={{ display: 'flex', background: '#f5f4f0', borderRadius: '10px', padding: '4px', marginBottom: '28px' }}>
            {[['giris', 'Giriş Yap'], ['kayit', 'Kayıt Ol']].map(([key, label]) => (
              <button key={key} onClick={() => { setMod(key); setHata(''); setMesaj('') }}
                style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', fontSize: '14px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', background: mod === key ? '#fff' : 'transparent', color: mod === key ? 'var(--text)' : 'var(--text-muted)', transition: 'all 0.15s' }}>
                {label}
              </button>
            ))}
          </div>
        ...
        {mod === 'sifre' && (
          <div style={{ marginBottom: '24px' }}>
            <button onClick={() => { setMod('giris'); setHata(''); setMesaj('') }}
              style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px', padding: 0, marginBottom: '12px' }}>
              ← Geri dön
            </button>
        ...
        {mod !== 'sifre' && (
          <>
            <button onClick={handleGoogle}
              style={{ width: '100%', padding: '11px', background: '#fff', border: '1px solid var(--border)', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: 500, fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '20px' }}>
        ...
        <form onSubmit={mod === 'giris' ? handleGiris : mod === 'kayit' ? handleKayit : handleSifre}>
          {mod === 'kayit' && (
            <div style={{ marginBottom: '14px' }}>
              ...
              <input value={form.kullaniciAdi} onChange={e => setForm(f => ({ ...f, kullaniciAdi: e.target.value }))}
                placeholder="kullanici_adi" required style={I} />
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
              placeholder="ornek@email.com" required style={I} />
          </div>
          {mod !== 'sifre' && (
            <div style={{ marginBottom: '14px' }}>
          ...
          {mod === 'kayit' && (
            <div style={{ marginBottom: '14px' }}>
              ...
              <input type="password" value={form.sifreTekrar} onChange={e => setForm(f => ({ ...f, sifreTekrar: e.target.value }))}
                placeholder="Şifreyi tekrar gir" required style={I} />
            </div>
          )}

          {hata && (
            <div style={{ fontSize: '13px', color: '#dc2626', marginBottom: '14px', padding: '10px 12px', background: '#fff0f0', borderRadius: '8px', border: '1px solid #fecaca' }}>
              {hata}
            </div>
          )}
          {mesaj && (
            <div style={{ fontSize: '13px', color: '#166534', marginBottom: '14px', padding: '10px 12px', background: '#f0fdf4', borderRadius: '8px', border: '1px solid #bbf7d0' }}>
              {mesaj}
            </div>
          )}

          <button type="submit" disabled={yukleniyor}
            style={{ width: '100%', padding: '12px', background: 'var(--text)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: 500, fontFamily: 'inherit', cursor: yukleniyor ? 'not-allowed' : 'pointer', opacity: yukleniyor ? 0.7 : 1 }}>
            {yukleniyor ? 'Lütfen bekle...' : mod === 'giris' ? 'Giriş Yap' : mod === 'kayit' ? 'Kayıt Ol' : 'Link Gönder'}
          </button>

          {mod === 'giris' && (
            <button type="button" onClick={() => { setMod('sifre'); setHata(''); setMesaj('') }}
              style={{ width: '100%', marginTop: '10px', background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px' }}>
              Şifremi unuttum
            </button>
          )}
        </form>
