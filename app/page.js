    ...
    <Link href={`/seri/${seri.slug}`} style={{ textDecoration: 'none' }}>
      ...
      >
        <div style={{
          ...
          {seri.kapak_url
            ? <img src={seri.kapak_url} alt={seri.baslik} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: hover ? 'scale(1.04)' : 'scale(1)', transition: 'transform 0.3s ease' }} loading="lazy" />
...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchAll() {
      const [{ data: sData }, { data: bData }, { data: ayarData }] = await Promise.all([
        supabase.from('seriler').select('*, kategoriler(isim)').order('created_at', { ascending: false }),
        supabase.from('bolumler').select('id, baslik, sayi, kapak_url, created_at, seri_id, seriler(baslik, slug, kapak_url)').order('created_at', { ascending: false }).limit(10),
        supabase.from('site_ayarlari').select('anahtar, deger').in('anahtar', ['anasayfa_hero_slider'])
      ])
      setSeriler(sData || [])
      setBolumler(bData || [])
      const ayarlarObj = {}
      ayarData?.forEach(item => { ayarlarObj[item.anahtar] = item.deger })
      setSiteAyarlari(ayarlarObj)
      setLoading(false)
    }
    fetchAll()
  }, [])
